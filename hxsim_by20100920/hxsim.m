...
function [HDmatrix, Fmatrix, obsPeaks]=hxsim(pepPara,hxPara)
...
[peptideMass, distND, maxND, maxD]=pepinfo(proSeq(START:END)); %call pepinfo.m
...
subplot(2,1,2)
obsPeaks(1,1)=0;
obsPeaks(1,2)=Distr(1);
for i=2:(maxD+1)
    obsPeaks(i,1)=obsPeaks(i-1,1)+1/Charge;
    obsPeaks(i,2)=Distr(i);
end
stem(obsPeaks(:,1), obsPeaks(:,2))
v=axis;
