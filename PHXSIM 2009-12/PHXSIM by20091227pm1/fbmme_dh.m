%%%2009-12-20 fbmme_dh.m: MATLAB implementation of FBMME_DH.XLS of
%%%Englander Lab for hydrogen exchange rates calculation

function kcDH = fbmme_dh(inputSeq, pH, TempC, molType)
...
pKc_Asp=-log10(10^(-3.87)*exp(-1*Ea_Asp*((1/Temp-1/278)/R)));
pKc_Glu=-log10(10^(-4.33)*exp(-1*Ea_Glu*((1/Temp-1/278)/R)));
pKc_His=-log10(10^(-7)*exp(-1*Ea_His*((1/Temp-1/278)/R)));
...
switch molType
    ...
    case 'oligo'
        ...
        Kw=Kw_oligo;
    otherwise
        error('Unknow molType!(must be "poly" or "oligo")')
end


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%reproduce FBMME_DH.XLS Table B:
%%% ASP COOH D+	d
%%% CYSTINE	C2	s
%%% GLU COOH E+	e
%%% PRO cis	Pc	p
...
Factors_list='ARDdNCsGEeQHILKMFPpSTWYVncma';
Factors_values=[0,0,0,0;...
    ...
    0,0.293000000000000,0,-0.197000000000000;];

Factors_values(3,1)=log10(10^(-0.9-pH)/(10^(-pKc_Asp)+10^(-pH))+10^(0.9-pKc_Asp)/(10^(-pKc_Asp)+10^(-pH)));
Factors_values(3,2)=log10(10^(-0.12-pH)/(10^(-pKc_Asp)+10^(-pH))+10^(0.58-pKc_Asp)/(10^(-pKc_Asp)+10^(-pH)));
Factors_values(3,3)=log10(10^(0.69-pH)/(10^(-pKc_Asp)+10^(-pH))+10^(-0.3-pKc_Asp)/(10^(-pKc_Asp)+10^(-pH)));
Factors_values(3,4)=log10(10^(0.6-pH)/(10^(-pKc_Asp)+10^(-pH))+10^(-0.18-pKc_Asp)/(10^(-pKc_Asp)+10^(-pH)));

Factors_values(9,1)=log10(10^(-0.6-pH)/(10^(-pKc_Glu)+10^(-pH))+10^(-0.9-pKc_Glu)/(10^(-pKc_Glu)+10^(-pH)));
Factors_values(9,2)=log10(10^(-0.27-pH)/(10^(-pKc_Glu)+10^(-pH))+10^(0.31-pKc_Glu)/(10^(-pKc_Glu)+10^(-pH)));
Factors_values(9,3)=log10(10^(0.24-pH)/(10^(-pKc_Glu)+10^(-pH))+10^(-0.51-pKc_Glu)/(10^(-pKc_Glu)+10^(-pH)));
Factors_values(9,4)=log10(10^(0.39-pH)/(10^(-pKc_Glu)+10^(-pH))+10^(-0.15-pKc_Glu)/(10^(-pKc_Glu)+10^(-pH)));

Factors_values(12,1)=log10(10^(-0.8-pH)/(10^(-pKc_His)+10^(-pH))+10^(0-pKc_His)/(10^(-pKc_His)+10^(-pH)));
Factors_values(12,2)=log10(10^(-0.51-pH)/(10^(-pKc_His)+10^(-pH))+10^(0-pKc_His)/(10^(-pKc_His)+10^(-pH)));
Factors_values(12,3)=log10(10^(0.8-pH)/(10^(-pKc_His)+10^(-pH))+10^(-0.1-pKc_His)/(10^(-pKc_His)+10^(-pH)));
Factors_values(12,4)=log10(10^(0.83-pH)/(10^(-pKc_His)+10^(-pH))+10^(0.14-pKc_His)/(10^(-pKc_His)+10^(-pH)));

Factors_values(26,1)=log10(10^(0.05-pH)/(10^(-pKc_Glu)+10^(-pH))+10^(0.96-pKc_Glu)/(10^(-pKc_Glu)+10^(-pH)));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%calculate kc(D->H) of each position:

Seq=['n', inputSeq, 'c'];
N=size(Seq,2); %residue numbers of the input peptide/protein plus 2(N- & C-term)
kcDH=zeros(N,1);
for i=1:N
    if i==1 || Seq(i)=='P' || Seq(i)=='p' || Seq(i)=='a'
        ...
        if Seq(i)=='P' || Seq(i)=='a' || i==1 || Seq(i)=='p' || i==N || i-1==1
            ...
            j=find(Factors_list==Seq(i));
            k=find(Factors_list==Seq(i-1));
