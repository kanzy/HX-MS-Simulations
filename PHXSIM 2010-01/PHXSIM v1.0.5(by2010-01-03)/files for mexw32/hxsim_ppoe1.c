    ...
    for (step=1; step<=simSteps; step++) {
        ...
        for (i=0; i<M; i++) {
            ...
            for (j=START-1; j<END; j++) {
                ran1=Random();
                switch ((int)HDmatrix[i*N+j-(int)START+1]) {
                    case 0: /* H */
                        if (ran1 <= (1-exp(-kcHD[j]*deltaT/pf[j]))*Dfraction)
                            HDmatrix[i*N+j-(int)START+1]=1; /* H->D */
                    case 1: /* D */
                        if (ran1 <= (1-exp(-kcDH[j]*deltaT/pf[j]))*Hfraction)
                            HDmatrix[i*N+j-(int)START+1]=0; /* D->H */
                }
            }
            ...
            if (flagSim!=1) {
                ...
                switch ((int)FoldStates[i]) {
                    case 0: /* U */
                        if (ran2 <= 1-exp(-k_fm[0]*deltaT))
                            FoldStates[i]=1; /* U->I */
                    case 1: /* I */ {
                        if (ran2 <= 1-exp(-k_fm[1]*deltaT))
                            FoldStates[i]=0; /* I->U */
                        if (ran2 > 1-exp(-k_fm[1]*deltaT) && ran2 <= (1-exp(-k_fm[1]*deltaT))+(1-exp(-k_fm[4]*deltaT)))
                            FoldStates[i]=3; /* I->N */
                        if (ran2>(1-exp(-k_fm[1]*deltaT))+(1-exp(-k_fm[4]*deltaT)) && ran2<=(1-exp(-k_fm[1]*deltaT))+(1-exp(-k_fm[4]*deltaT))+(1-exp(-k_fm[2]*deltaT)))
                            FoldStates[i]=2; /* I->Ix */}
                    case 2: /* Ix */
                        if (ran2 <= 1-exp(-k_fm[3]*deltaT))
                            FoldStates[i]=1; /* Ix->I */
                    case 3: /* N */
                        if (ran2 <= 1-exp(-k_fm[5]*deltaT))
                            FoldStates[i]=1; /* N->I */
                }
            }
        }
